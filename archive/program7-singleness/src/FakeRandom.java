...
public class FakeRandom extends RandomDice {
	 ...
	 */
	public FakeRandom(int n, String s) {
		super(n);
	 ...
	 */
	public FakeRandom(int n, FileInputStream f){
		super(n);
		this.stream = f;
		this.scan = new Scanner(stream);
	}
	...
	public int roll() {
		...
		if (result > sides || result < 1) {
			int raw = result;
			result = (raw % sides) + 1;
			System.out.printf("WARNING: obtained value %d is greater than number of sides (%d). Returning %d.\n", raw, sides, result);
		}
		return result;
	}
}
